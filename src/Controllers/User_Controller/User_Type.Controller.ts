...
const User_Type_Controller = {
    ...
    async getAllUserType(req: Request, res: Response) {
        try {
            // Fetch user types with lean() for performance
            const userTypes = await UserTypeModel.find().lean();

            // Check if no user types were found
            if (!userTypes.length) {
            ...
            console.error('User_Type_Getting_Error:', error);
